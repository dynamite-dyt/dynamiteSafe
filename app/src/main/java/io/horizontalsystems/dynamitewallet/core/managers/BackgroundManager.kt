...
class BackgroundManager(application: Application) : Application.ActivityLifecycleCallbacks {
    ...
    override fun onActivityStarted(activity: Activity?) {
        if(refs == 0) {
            io.horizontalsystems.dynamitewallet.core.App.lockManager.willEnterForeground()
        }
        refs++
    }
    ...
    override fun onActivityStopped(activity: Activity?) {
        ...
        if (refs == 0) {
            ...
            io.horizontalsystems.dynamitewallet.core.App.lockManager.didEnterBackground()
        }
    }
