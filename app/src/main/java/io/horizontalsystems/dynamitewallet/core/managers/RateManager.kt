                ...
                networkManager.getLatestRateData(ServiceExchangeApi.HostType.MAIN, currencyCode)
                        ...
                        .subscribe({ latestRateData ->
                            coinCodes.forEach { coinCode ->
                                if(coinCode == "DYT"){
                                    networkManager.getLastestRateDataDYT().subscribeOn(Schedulers.io())
                                            .observeOn(Schedulers.io())
                                            .subscribe({ latestRateData ->
                                                if(latestRateData != null) {
                                                    val rate = Rate(coinCode, "USD", (latestRateData.get("price") as JsonObject).get("priceUSD").asBigDecimal, java.util.Calendar.getInstance().timeInMillis, true)
