...
class BitcoinCashAdapter(coin: Coin, override val kit: BitcoinCashKit, addressParser: AddressParser, private val feeRateProvider: io.horizontalsystems.dynamitewallet.core.IFeeRateProvider)
    : io.horizontalsystems.dynamitewallet.core.BitcoinBaseAdapter(coin, kit, addressParser), BitcoinCashKit.Listener {
    ...
    constructor(coin: Coin, authData: AuthData, syncMode: SyncMode, testMode: Boolean, feeRateProvider: io.horizontalsystems.dynamitewallet.core.IFeeRateProvider) :
            this(coin, io.horizontalsystems.dynamitewallet.core.BitcoinCashAdapter.Companion.createKit(authData, syncMode, testMode), AddressParser("bitcoincash", false), feeRateProvider)
    ...
    companion object {
        ...
        private fun createKit(authData: AuthData, syncMode: SyncMode, testMode: Boolean): BitcoinCashKit {
            return BitcoinCashKit(io.horizontalsystems.dynamitewallet.core.App.Companion.instance, authData.words, authData.walletId, syncMode = syncMode.bitcoinKitMode(), networkType = io.horizontalsystems.dynamitewallet.core.BitcoinCashAdapter.Companion.getNetworkType(testMode))
        }

        fun clear(context: Context, walletId: String, testMode: Boolean) {
            BitcoinCashKit.clear(context, io.horizontalsystems.dynamitewallet.core.BitcoinCashAdapter.Companion.getNetworkType(testMode), walletId)
        }
    }
}
