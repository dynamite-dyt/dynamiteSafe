    ...
    private fun setTransactionInfoDialog() {
        ...
        viewModel.transactionLiveData.observe(lifecycleOwner, Observer { txRecord ->
            txRecord?.let { txRec ->
                ...
                fiatValue.apply {
                    text = txRec.currencyValue?.let { io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(it, showNegativeSign = true, canUseLessSymbol = false) }
                    setTextColor(resources.getColor(if (txRec.incoming) R.color.white else R.color.yellow_crypto, null))
                }

                coinValue.text = io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(txRec.coinValue, explicitSign = true, realNumber = true)
                txInfoCoinName.text = txRec.coin.title

                itemRate.apply {
                    txRec.rate?.let {
                        val rate = context.getString(R.string.Balance_RatePerCoin, io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(it, canUseLessSymbol = false), txRec.coin.code)
