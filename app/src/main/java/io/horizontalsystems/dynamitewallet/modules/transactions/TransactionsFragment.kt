...
class ViewHolderTransaction(override val containerView: View, private val l: ClickListener) : ViewHolder(containerView), LayoutContainer {
    ...
    fun bind(transactionRecord: TransactionViewItem, showBottomShade: Boolean) {
        txValueInFiat.text = transactionRecord.currencyValue?.let {
            io.horizontalsystems.dynamitewallet.core.App.numberFormatter.formatForTransactions(it, transactionRecord.incoming)
        }

        if ( transactionRecord.coinValue.coinCode === "DYT" ) {
            coinIconTx.setImageResource(R.drawable.dyt_logo)
        }
        else {
            coinIconTx.setImageResource(R.drawable.coin_eth)
        }
        txValueInCoin.text = io.horizontalsystems.dynamitewallet.core.App.numberFormatter.formatForTransactions(transactionRecord.coinValue)
