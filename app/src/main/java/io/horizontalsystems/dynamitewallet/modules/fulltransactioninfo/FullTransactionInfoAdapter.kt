...
import io.horizontalsystems.dynamitewallet.entities.*
...
class FullTransactionBitcoinAdapter(val provider: FullTransactionInfoModule.BitcoinForksProvider, val coin: Coin, val unitName: String)
    ...
    override fun convert(json: JsonObject): FullTransactionRecord {
        ...
        val transactionItems = mutableListOf(FullTransactionItem(R.string.FullInfo_Fee, value = "${io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(data.fee)} ${coin.code}"))
        ...
        data.feePerByte?.let { feePerByte ->
            transactionItems.add(FullTransactionItem(R.string.FullInfo_Rate, value = "${io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(feePerByte)} ($unitName)", dimmed = true))
        }

        sections.add(FullTransactionSection(items = transactionItems))

        if (data.inputs.isNotEmpty()) {
            val totalInput = io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(data.inputs.sumByDouble { it.value })
            val inputs = mutableListOf(FullTransactionItem(R.string.FullInfo_SubtitleInputs, value = "$totalInput ${coin.code}"))
            data.inputs.map {
                val amount = io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(it.value)
        ...
        if (data.outputs.isNotEmpty()) {
            val totalOutput = io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(data.outputs.sumByDouble { it.value })
...
class FullTransactionEthereumAdapter(val provider: FullTransactionInfoModule.EthereumForksProvider, val coin: Coin)
    ...
    override fun convert(json: JsonObject): FullTransactionRecord {
        ...
        mutableListOf<FullTransactionItem>().let { section ->
            ...
            section.add(FullTransactionItem(R.string.FullInfoEth_Amount, value = "${io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(amount)} ${coin.code}"))
        ...
        mutableListOf<FullTransactionItem>().let { section ->
            data.fee?.let {
                section.add(FullTransactionItem(R.string.FullInfo_Fee, value = "${io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(it.toDouble())} ETH"))
            }
            if (data.size != null) {
