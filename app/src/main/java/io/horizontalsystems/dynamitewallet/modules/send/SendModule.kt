...
import io.horizontalsystems.dynamitewallet.core.FeeRatePriority
...
object SendModule {
    ...
    fun init(view: SendViewModel, coinCode: String) {
        val adapter = io.horizontalsystems.dynamitewallet.core.App.adapterManager.adapters.first { it.coin.code == coinCode }
        val interactor = SendInteractor(io.horizontalsystems.dynamitewallet.core.App.currencyManager, io.horizontalsystems.dynamitewallet.core.App.rateStorage, io.horizontalsystems.dynamitewallet.core.App.localStorage, TextHelper, adapter, io.horizontalsystems.dynamitewallet.core.App.appConfigProvider)
    ...
    sealed class AmountInfo {
        ...
        fun getFormatted(): String? = when (this) {
            is CoinValueInfo -> {
                io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(this.coinValue)
            }
            is CurrencyValueInfo -> {
                io.horizontalsystems.dynamitewallet.core.App.numberFormatter.format(this.currencyValue, trimmable = true)
            }
        }
    }
